...
var handleCheckPageLoadUrl = function(url) {
    url = (url) ? url : 'home';
    if (url === '') {
        //$('#ajax-content').html(default_content);
    } else {
        var path = url.replace(basePath, "");
        ...
        var fpath = $("#sidebar a[href*='"+route+"']");
        if(fpath.length > 0)
            fpath.parents('li').addClass('active');
        else
...
function showMessage(type) {
    ...
    switch (type){
        ...
        case 'error':
            ...
            content = 'Error happened on server side please contact admin.';
            break;
        default:
            className = 'alert-success';
    ...
    html = '<div class="alert '+className+'">\n' +
        '                        <strong><i class="fa fa-check"></i></strong> '+content+'\n' +
        '                        <span class="close" data-dismiss="alert">×</span>\n' +
        '                    </div>';
    $("#messages").html(html).slideDown('slow');
}

function clickActionHanlder()
{
...
var App = function () {
	...
	return {
		...
		init: function () {
		    ...
		    clickActionHanlder();
            handleCheckPageLoadUrl(window.location.href);
		    this.initSidebar();
		    this.initTopMenu();
		    this.initPageLoad();
		},
